...
void WildshapeAbilityBuffs(object oPC, float fDuration, object oRightHand = OBJECT_INVALID, object oLeftHand = OBJECT_INVALID)
{
	...
	if (GetHasSpellEffect(1002, oPC))
	{
		...
		itemproperty iNatCritUnarm = ItemPropertyBonusFeat(IPRP_FEAT_IMPCRITCREATURE);		
				if (GetIsObjectValid(oCWeapon1))
				{
					if (!IPGetIsBludgeoningWeapon(oCWeapon1))
					{
						IPSafeAddItemProperty(oCWeapon1, iKeen, fSpellDuration, X2_IP_ADDPROP_POLICY_REPLACE_EXISTING);
						IPSafeAddItemProperty(oCWeapon1, iImpCritUnarm, fSpellDuration, X2_IP_ADDPROP_POLICY_REPLACE_EXISTING);
						IPSafeAddItemProperty(oCWeapon1, iNatCritUnarm, fSpellDuration, X2_IP_ADDPROP_POLICY_REPLACE_EXISTING);												
					}
				}
				else if (GetIsObjectValid(oCWeapon2))
				{
					if (!IPGetIsBludgeoningWeapon(oCWeapon2))
					{
						IPSafeAddItemProperty(oCWeapon2, iKeen, fSpellDuration, X2_IP_ADDPROP_POLICY_REPLACE_EXISTING);
						IPSafeAddItemProperty(oCWeapon2, iImpCritUnarm, fSpellDuration, X2_IP_ADDPROP_POLICY_REPLACE_EXISTING);
						IPSafeAddItemProperty(oCWeapon2, iNatCritUnarm, fSpellDuration, X2_IP_ADDPROP_POLICY_REPLACE_EXISTING);						
					}
				}
				else if (GetIsObjectValid(oCWeapon3))
				{
					if (!IPGetIsBludgeoningWeapon(oCWeapon3))
					{
						IPSafeAddItemProperty(oCWeapon3, iKeen, fSpellDuration, X2_IP_ADDPROP_POLICY_REPLACE_EXISTING);
						IPSafeAddItemProperty(oCWeapon3, iImpCritUnarm, fSpellDuration, X2_IP_ADDPROP_POLICY_REPLACE_EXISTING);
						IPSafeAddItemProperty(oCWeapon3, iNatCritUnarm, fSpellDuration, X2_IP_ADDPROP_POLICY_REPLACE_EXISTING);						
					}
				}					
	}
