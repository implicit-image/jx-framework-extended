...
//::
//::             Reeron modified on 7-19-07
//::             Added ability for Greater Dispel to remove curses as per PnP.
//::
...
// RPGplayer1 1/8/09 - spellsDispelAoE() won't remove auras anymore, since spellsDispelMagic() already handles them
// RPGplayer1 1/11/09 - updated spellsDispelAoE(), to better match 3.5e rules (and avoids use of GetCasterAbilityModifier(), which is bad for divine casters)
// RPGplayer1 1/22/09 - modified spellApplyMindBlank(), so it removes Tasha's Hideous Laughter and Solipsism
// Drammel 5/15/2009 - Merged Kaedrin's and RPGplayer1's verions of this file as best as I could.  Also added support for Mettle in spellsInflictTouchAttack.
// Drammel 6/19/2009 - Added Bot9sReflexAdjustedDamage here to aid with a few maneuvers.
...
//  doAura(), DoCamoflage(), DoDirgeEffect(), DoGrenade(), DoMagicFang(), DoMissileStorm(), DoSpikeGrowthEffect(), GetCappedCasterLevel(),
...
void DoTrapSpike(int nDamage)
{
    ...
    if (nDamage > 0)
    {
        ...
        JXApplyEffectAtLocation(DURATION_TYPE_INSTANT, eVis, GetLocation(oTarget));
...
//        SpawnScriptDebugger();
    ...
    if (nTouch != TOUCH_ATTACK_RESULT_MISS)
    {
        ...
        // * must be the correct racial type (only used with Holy Water)
            if ((nRacialType != RACIAL_TYPE_ALL) && (nRacialType == GetRacialType(oTarget)))
            {
                ...
                //JXApplyEffectToObject(DURATION_TYPE_TEMPORARY, eVis, oTarget); VISUALS outrace the grenade, looks bad
            }
            else
    ...
    //    JXApplyEffectToObject(DURATION_TYPE_INSTANT, eMissile, oTarget);
    }
...
//    int nMetaMagic = GetMetaMagicFeat();
   ...
   // SpawnScriptDebugger();
    ...
    if (spellsIsTarget(oTarget, SPELL_TARGET_SELECTIVEHOSTILE, JXGetAOERealCreator()))
    {
        //Fire cast spell at event for the target
        SignalEvent(oTarget, EventSpellCastAt(JXGetAOERealCreator(), JXGetSpellId()));
        //Spell resistance check
        if(!MyResistSpell(JXGetAOERealCreator(), oTarget))
        {

            //Make a Fortitude Save to avoid the effects of the movement hit.
            if(!MySavingThrow(SAVING_THROW_FORT, oTarget, JXGetSpellSaveDC(), SAVING_THROW_ALL, JXGetAOERealCreator()))
            {
...
void DoSpikeGrowthEffect(object oTarget)
{
    ...
    {
        //Fire cast spell at event for the target
        SignalEvent(oTarget, EventSpellCastAt(JXGetAOERealCreator(), SPELL_SPIKE_GROWTH));
        //Spell resistance check
        if(!MyResistSpell(JXGetAOERealCreator(), oTarget, fDelay))
        {
            int nMetaMagic = JXGetMetaMagicFeat();
            ...
            //Apply damage and visuals
            //DelayCommand(fDelay, JXApplyEffectToObject(DURATION_TYPE_INSTANT, eVis, oTarget));
            DelayCommand(fDelay, JXApplyEffectToObject(DURATION_TYPE_INSTANT, eDam/*eLink*/, oTarget));
...
void DoCaltropEffect(object oTarget)
{
 ...
 //   effect eVis = EffectVisualEffect(VFX_IMP_SPIKE_TRAP);
    ...
    {
        //Fire cast spell at event for the target
        SignalEvent(oTarget, EventSpellCastAt(JXGetAOERealCreator(), 471));
        {
            effect eDam = EffectDamage(1, DAMAGE_TYPE_PIERCING);
            float fDelay = GetRandomDelay(1.0, 2.2);
            //Apply damage and visuals
            //DelayCommand(fDelay, JXApplyEffectAtLocation(DURATION_TYPE_INSTANT, eVis, GetLocation(oTarget)));
            DelayCommand(fDelay, JXApplyEffectToObject(DURATION_TYPE_INSTANT, eDam, oTarget));
            ...
            if (!MySavingThrow(SAVING_THROW_FORT, oTarget, nSaveDC))
            {
                ...
                if (bIsPC == TRUE)
                {
                    if (bShowPopup == TRUE)
                    {
                        ...
                        JXApplyEffectToObject(DURATION_TYPE_TEMPORARY, eLink, oTarget, fDifficulty);
                }
                else
                {
                    JXApplyEffectToObject(DURATION_TYPE_PERMANENT, eLink, oTarget);
        ...
        )
{
    ...
    {
        JXApplyEffectAtLocation(DURATION_TYPE_INSTANT, eImpact, lTargetLoc);
    }
    ...
    while(GetIsObjectValid(oTarget))
    {
        if (spellsIsTarget(oTarget, nTargetType, oCaster) == TRUE)
        {
          ...
          if(!nResistSpellSuccess)
          {
                ...
                {
                  nSavingThrowSuccess = MySavingThrow(nSavingThrowType, oTarget, JXGetSpellSaveDC(), nSavingThrowSubType);
                }
                   ...
                   // * Had to remove this test because LINKED effects have no valid type.
                    {
                        ...
                        DelayCommand(fDelay, JXApplyEffectToObject(nDurationType, eLink, oTarget, fDuration));
                    }
...
void spellApplyMindBlank(object oTarget, int nSpellId, float fDelay=0.0)
{
    ...
    int bValid;
    float fDuration = TurnsToSeconds(JXGetCasterLevel(OBJECT_SELF));
...
void doAura(int nAlign, int nVis1, int nVis2, int nDamageType)
{
    //Declare major variables
    object oTarget = JXGetSpellTargetObject();
    float fDuration = RoundsToSeconds(JXGetCasterLevel(OBJECT_SELF));
...
void spellsDispelMagic(object oTarget, int nCasterLevel, effect eVis, effect eImpac, int bAll = TRUE, int bBreachSpells = FALSE)
{
    ...
    if (bAll == TRUE )
    {
        if ( nId == SPELL_GREATER_DISPELLING) // Reeron added so this spell follows PnP ability to remove curses.
        {
            int nEffect1 = EFFECT_TYPE_CURSE;
            int nCurse = GetLocalInt(oTarget, "Saved Curse Level");
            if (nCurse>0)
            {
                eVis = EffectVisualEffect(VFX_HIT_SPELL_ABJURATION);
                int nCasterRoll = nCasterLevel + d20();
                //FloatingTextStringOnCreature("nCasterRoll = "+IntToString(nCasterRoll), oTarget);
                int nDC = 11 + nCurse;
                //FloatingTextStringOnCreature("nDC = "+IntToString(nDC), oTarget);
                if (((nCasterRoll>= nDC )))
                {
                    RemoveSpecificEffect(nEffect1, oTarget);
                    FloatingTextStringOnCreature("Removed curse using PnP dispel logic.", oTarget);
                    DeleteLocalInt(oTarget, "Saved Curse Level");
                }
    ...
    DelayCommand(fDelay, JXApplyEffectToObject(DURATION_TYPE_INSTANT, eVis, oTarget));
    DelayCommand(fDelay, JXApplyEffectToObject(DURATION_TYPE_INSTANT, eDispel, oTarget));
}
...
// now
//------------------------------------------------------------------------------
// Reworked to be closer to 3.5e rules:
//   Base chance is 50%, if both caster and AoE have same caster level.
//   Adjust by +/-5%, for caster level difference between the two.
//   If no caster level can be found for AoE, use deafault value of 10.
//
// This should match 1d20 + caster level check vs DC of 11 + AoE's caster level
//------------------------------------------------------------------------------
void spellsDispelAoE(object oTargetAoE, object oCaster, int nCasterLevel)
{
    //auras on cratures have permanent AOE object, and spellsDispelMagic() already handles them
    if (GetAreaOfEffectDuration(oTargetAoE) == DURATION_TYPE_PERMANENT)
        return;
