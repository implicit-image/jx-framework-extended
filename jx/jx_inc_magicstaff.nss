...
// Use the magic staff system. The staff wielder's caster level is used if it is better
// than the staff's caster level. The wielder's spell save DC is also used.
...
SendMessageToPC(GetFirstPC(), "item caster level : " + IntToString(iItemCasterLevel));
		// Get the caster level of the staff's wielder (it may depend on the spell)
		int iWielderCasterLevel = JXGetCreatureCasterLevelForSpell(iSpellId, oWielder);
		//if (iWielderCasterLevel == 0)
		//	iWielderCasterLevel = JXGetCreatureCasterLevel(oWielder);
SendMessageToPC(GetFirstPC(), "wielder caster level : " + IntToString(iWielderCasterLevel));
...
SendMessageToPC(GetFirstPC(), "caster level set : " + IntToString(JXGetCasterLevel()));
...
SendMessageToPC(GetFirstPC(), "save DC set : " + IntToString(JXGetSpellSaveDC()));
	}
}