...
// ChazM 1/8/07 - Added JXEffectFatigue(), modified ApplyFatigue()
// ChazM 5/10/07 - Added JXEffectExhausted(), JXEffectSickened()
...
//void main(){}
// jallaix 06/18/2007 - Replaced GetCasterLevel() and GetMetaMagicFeat() by their JX versions
...
{
        int     nHitpoints      = JXGetCasterLevel(oCaster) * 2;
        effect  eHitPoints      = JXEffectTemporaryHitpoints( nHitpoints );
        ...
        JXApplyEffectToObject(DURATION_TYPE_TEMPORARY, eHitPoints, oCaster, fDuration);
}
...
{
        int             nDamage = JXGetCasterLevel( oCaster ) / 2;
        effect  eDmg    = JXEffectDamage( nDamage );
...
{
        effect eAttackBonus = JXEffectAttackIncrease(2);
        effect eSaveBonus   = JXEffectSavingThrowIncrease(SAVING_THROW_ALL, 2);
        effect eLink        = JXEffectLinkEffects(eAttackBonus, eSaveBonus);
        float  fDuration        = RoundsToSeconds(10); // 10 rounds
        JXApplyEffectToObject(DURATION_TYPE_TEMPORARY, eLink, oCaster, fDuration);
}
...
void DispelMagicWithCallback( object oTarget, object oCaster, int nCasterLevel, effect eVis, effect eImpac, int bAll, int nSpellID )
{
    ...
    JXApplyEffectToObject(DURATION_TYPE_INSTANT, eDispel, oTarget);
}
...
int AttemptNewSong( object oCaster, int bIsPersistent = FALSE )
{
    ...
    if(bIsPersistent)
    {
        ...
        if(nSingingSpellId != JXGetSpellId())
        {
            // Now add new singing Effect
            effect eBardSong = JXEffectBardSongSinging(JXGetSpellId());
            JXApplyEffectToObject(DURATION_TYPE_PERMANENT, eBardSong, oCaster);
            return TRUE;
        }
        return FALSE;
    }
...
void ApplyFriendlySongEffectsToTarget( object oCaster, int nSpellId, float fDuration, effect eLink )
{
    ...
    effect eVis    = ExtraordinaryEffect( JXEffectVisualEffect(VFX_DUR_BARD_SONG) );

    effect eImpact = ExtraordinaryEffect( JXEffectVisualEffect(VFX_HIT_SPELL_SONIC) );  // AFW-OEI 07/18/2007: NWN2 VFX
    effect eFNF    = ExtraordinaryEffect( JXEffectVisualEffect(VFX_FNF_LOS_NORMAL_30) );
    JXApplyEffectAtLocation(DURATION_TYPE_INSTANT, eFNF, GetLocation(oCaster));
    ...
    if(GetIsObjectValid(oTarget))
    {
                ...
                if(!GetHasSpellEffect(nSpellId,oTarget))
                {
                     ...
                     //{
                        ...
                        if(oTarget == oCaster)
                        {
                            effect eLinkBard = ExtraordinaryEffect( JXEffectLinkEffects(eLink, eVis) );
                            eLinkBard        = SetEffectSpellId( eLinkBard, nSpellId );
                            JXApplyEffectToObject(DURATION_TYPE_TEMPORARY, eLinkBard, oTarget, fDuration);
                        }
                                        ...
                                        else if ( spellsIsTarget(oTarget, SPELL_TARGET_ALLALLIES, oCaster) )
                        {
                            ...
                            JXApplyEffectToObject(DURATION_TYPE_INSTANT, eImpact, oTarget);
                            JXApplyEffectToObject(DURATION_TYPE_TEMPORARY, eLink, oTarget, fDuration);
                        }
                    //}
                }
...
void ExecuteDefaultMeteorSwarmBehavior( object oTarget, location lTarget )
{
    ...
    effect eVis = JXEffectVisualEffect( VFX_HIT_SPELL_METEOR_SWARM );
        float fTravelTime;
        ...
        DelayCommand( (fDelay2 + fTravelTime), JXApplyEffectAtLocation(DURATION_TYPE_INSTANT, eVis, lAhead) );
        ...
        DelayCommand( (fDelay2 + fTravelTime), JXApplyEffectAtLocation(DURATION_TYPE_INSTANT, eVis, lBehind) );
        ...
        DelayCommand( (fDelay2 + fTravelTime), JXApplyEffectAtLocation(DURATION_TYPE_INSTANT, eVis, lLeft) );
        ...
        DelayCommand( (fDelay2 + fTravelTime), JXApplyEffectAtLocation(DURATION_TYPE_INSTANT, eVis, lRight) );
        ...
        DelayCommand( (fDelay2 + fTravelTime), JXApplyEffectAtLocation(DURATION_TYPE_INSTANT, eVis, lAheadLeft) );
        ...
        DelayCommand( (fDelay2 + fTravelTime), JXApplyEffectAtLocation(DURATION_TYPE_INSTANT, eVis, lAheadRight) );
        ...
        DelayCommand( (fDelay2 + fTravelTime), JXApplyEffectAtLocation(DURATION_TYPE_INSTANT, eVis, lFlankingLeft) );
        ...
        DelayCommand( (fDelay2 + fTravelTime), JXApplyEffectAtLocation(DURATION_TYPE_INSTANT, eVis, lFlankingRight) );
...
// Simulated JXEffects
//-----------------------------------------------------------
// FIXME: commented out, duplicates of jx_inc_magic_effects.nss
// Simulates a fatigue effect.  Can't be dispelled.
// effect JXEffectFatigue()
// {
//         // Create the fatigue penalty
//         effect eStrPenalty = JXEffectAbilityDecrease(ABILITY_STRENGTH, 2);
//         effect eDexPenalty = JXEffectAbilityDecrease(ABILITY_DEXTERITY, 2);
//         effect eMovePenalty = JXEffectMovementSpeedDecrease(10);        // 10% decrease

//         effect eRet = JXEffectLinkEffects (eStrPenalty, eDexPenalty);
//         eRet = JXEffectLinkEffects(eRet, eMovePenalty);
...
// {
//         effect eStrPenalty = JXEffectAbilityDecrease(ABILITY_STRENGTH, 6);
//         effect eDexPenalty = JXEffectAbilityDecrease(ABILITY_DEXTERITY, 6);
//         effect eMovePenalty = JXEffectMovementSpeedDecrease(50);        // 50% decrease
...
// {
//         effect eAttackPenalty = JXEffectAttackDecrease(2);
//         effect eDamagePenalty = JXEffectDamageDecrease(2);
//         effect eSavePenalty = JXEffectSavingThrowDecrease(SAVING_THROW_ALL, 2);
//         effect eSkillPenalty = JXEffectSkillDecrease(SKILL_ALL_SKILLS, 2);
//         //effect eAbilityPenalty = JXEffectAbilityDecrease(, 2);

//         effect eRet = JXEffectLinkEffects (eAttackPenalty, eDamagePenalty);
//         eRet = JXEffectLinkEffects(eRet, eSavePenalty);
//         eRet = JXEffectLinkEffects(eRet, eSkillPenalty);
//         eRet = ExtraordinaryEffect(eRet);
//         return (eRet);
// }
...
void ApplyFatigue(object oTarget, int nFatigueDuration, float fDelay = 0.0f)
{
        ...
        if( !GetIsResting() )
        {
                ...
                DelayCommand(fDelay, JXApplyEffectToObject(DURATION_TYPE_TEMPORARY, eFatigue, oTarget, fFatigueDuration));
        }
}
...
int HealHarmObject( object oTarget, effect eVis, effect eVis2, int nCasterLvl, int nSpellId )
{
        ...
        int nZombified = GetLocalInt(oTarget,"nZombified");
        if (((nRacialType == RACIAL_TYPE_UNDEAD || nZombified > 0) && spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, OBJECT_SELF))
                || ((nRacialType != RACIAL_TYPE_UNDEAD && !nZombified) && spellsIsTarget(oTarget, SPELL_TARGET_ALLALLIES, OBJECT_SELF)))
        {
                bRet = TRUE;
...
/*
        ...
        if ((GetRacialType(oTarget) == RACIAL_TYPE_UNDEAD || nZombified > 0) && spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, OBJECT_SELF))
        {
                DelayCommand(fDelay, HarmTarget( oTarget, OBJECT_SELF, nSpellId ) );
        }
        else if( GetRacialType(oTarget) != RACIAL_TYPE_UNDEAD && !nZombified)
        {
...
int HealHarmFaction( int nMaxToHealHarm, effect eVis, effect eVis2, int nCasterLvl, int nSpellId ) // returns the # HealHarmd
{
    ...
    {
        int nZombified = GetLocalInt(oTarget,"nZombified");
        if (GetRacialType(oTarget) != RACIAL_TYPE_UNDEAD && !nZombified)
        {
...
int HealHarmNearby( int nMaxToHealHarm, effect eVis, effect eVis2, int nCasterLvl, int nSpellId ) // returns the # HealHarmd
{
    ...
    object oTarget = GetFirstObjectInShape(SHAPE_SPHERE, RADIUS_SIZE_LARGE, JXGetSpellTargetLocation());
    while ( GetIsObjectValid(oTarget) && nNumEffected < nMaxToHealHarm )
    {
                int nZombified = GetLocalInt(oTarget,"nZombified");
                if ( !GetFactionEqual( oTarget, OBJECT_SELF ) || GetRacialType(oTarget) == RACIAL_TYPE_UNDEAD || nZombified > 0 ) // We've already done faction characters
                {
                        // this won't effect oTarget unless it is a friendly non-undead or enemy undead
        ...
        oTarget = GetNextObjectInShape(SHAPE_SPHERE, RADIUS_SIZE_LARGE, JXGetSpellTargetLocation());
    }
