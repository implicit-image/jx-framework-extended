//:://////////////////////////////////////////////////
//:: tob_x0_i0_enemy based on X0_I0_ENEMY
...
//:://////////////////////////////////////////////////
// ChazM 1/16/07 - EvenFlw's mods: CheckFriendlyFire() - defualt disatnace changed to 7.5;
//				 	GetNumberOfMeleeAttackers() - attaker count include those not in LOS, now sets enemyclose flag
// 					CheckEnemyGroupingOnTarget() - takes placeable into account
// 					added GetNearestNonEnemy()
//					x0_i0_assoc include moved here (from x0_i0_equip)
...
const string EVENFLW_AI_CLOSE 			= "EVENFLW_AI_CLOSE";
...
int CheckFriendlyFireOnTarget(object oTarget, float fDistance = 7.5);
    ...
    while(GetIsObjectValid(oTarget)) {
        if(!GetIsDead(oTarget)) {
			if(GetIsEnemy(oTarget)) {
            	nCnt++;
				if (!bEnemyClose)
					if(GetDistanceBetween(OBJECT_SELF, oTarget) < 2.3f)
						bEnemyClose = TRUE;
			}
		}
        oTarget = GetNextObjectInShape(SHAPE_SPHERE, 5.0, GetLocation(OBJECT_SELF), FALSE);
    }
	if(bEnemyClose) 
		SetLocalInt(OBJECT_SELF, EVENFLW_AI_CLOSE, TRUE);
	else 
...
int CheckEnemyGroupingOnTarget(object oTarget, float fDistance = 7.5)
{
    int nCnt;
	int bAvoidObjects = FALSE;
	
	if (GetAssociateState(NW_ASC_POWER_CASTING) 
		|| GetAssociateState(NW_ASC_SCALED_CASTING) )  
		bAvoidObjects = 1;
    ...
    while(GetIsObjectValid(oCheck))
    {
        if( GetIsEnemy(oCheck) 
			&& oTarget != oCheck 
			&& GetDistanceBetween(oTarget, oCheck) <= 5.0f)
        {
            nCnt++;
        } 
		// if we are avoiding objects and this is a non-plot placeable, then subtract 1 from the enemy count
		else if( bAvoidObjects 
				&& GetObjectType(oCheck)==OBJECT_TYPE_PLACEABLE 
				&& !GetPlotFlag(oCheck))
		{			
			nCnt--;
		}			
...
object GetNearestNonEnemy()
{
	object oFriend = GetFirstObjectInShape(SHAPE_SPHERE, RADIUS_SIZE_COLOSSAL, GetLocation(OBJECT_SELF), TRUE);
	while(GetIsObjectValid(oFriend)) {
		if(!GetIsEnemy(oFriend) && !GetAssociateState(NW_ASC_MODE_DYING, oFriend))
			break;
		oFriend = GetNextObjectInShape(SHAPE_SPHERE, RADIUS_SIZE_COLOSSAL, GetLocation(OBJECT_SELF), TRUE);
	}
	return oFriend;
}
