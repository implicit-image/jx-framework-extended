//:://////////////////////////////////////////////////
//:: tob_i0_assoc based on X0_I0_ASSOC
...
//:://////////////////////////////////////////////////
// ChazM 8/2/06 Modified GetFollowDistance() - NW_ASC_DISTANCE_2_METERS now set to 2.4 meters.
// DBR 08/03/06 - Added NW_ASC_MODE_PUPPET
// ChazM 8/18/06 - Added optional Target param to GetFollowDistance()
// ChazM 8/29/06 - Moved GetCurrentMaster() here from x0_inc_henai
// BDF 9/19/06 - Commented out the GetFactionEqual() check in GetCurrentMaster()
// ChazM 9/20/06 - Added/commented GetPCFactionLeader() 
...
const int NW_ASC_MODE_PUPPET = 		   0x00020000;
...
object GetCurrentMaster( object oFollower=OBJECT_SELF );
...
{
    float fDistance;
    if(GetAssociateState(NW_ASC_DISTANCE_2_METERS, oTarget))
    {
        fDistance = 2.4;
    }
    else if(GetAssociateState(NW_ASC_DISTANCE_4_METERS, oTarget))
    {
        fDistance = 4.0;
    }
    else if(GetAssociateState(NW_ASC_DISTANCE_6_METERS, oTarget))
    {
        fDistance = 6.0;
    }
...
/*
// Get the leader if PC faction or else OBJECT_INVALID
object GetPCFactionLeader(object oCreature)
{
	object oLeader = OBJECT_INVALID;
	// this condition needed until GetFactionLeader() updated
	if (GetFactionEqual(oCreature, GetFirstPC()) == TRUE )	// 9/19/06 - BDF: this conditional doesn't work in multiplayer muli-party
	{
		// This doesn't return OBJECT_INVALID, but the change request is in.
		oLeader = GetFactionLeader( oCreature );	// GetFactionLeader() returns OBJECT_INVALID for non-PC faction members
	}
	return (oLeader);
}
*/
...
object GetCurrentMaster( object oFollower=OBJECT_SELF )
{
	...
	if ( GetIsObjectValid(oMaster) == FALSE )
	{
		...
		// Similar to GetPCLeader() in ginc_companions
...
// * Returns master of henchmen or faction leader of PC party
object GetCurrentRealMaster( object oFollower=OBJECT_SELF )
{
    object oMaster = GetMaster( oFollower );
